# This code uses functions adapted from:
# Author: Ivan Milic
# Repository: https://github.com/ivanzmilic/2lvl_nlte.git
...
def calculate_J_L(tau, S, mu, wmu, profile, wx, line_ratio, B_boundary):
    ND = S.shape[0]
    NM = mu.shape[0]
    NL = profile.shape[0]
    J = np.zeros(ND, dtype=np.float64)
    L = np.zeros(ND, dtype=np.float64)
    tau_line = tau * line_ratio

    for m in range(NM):
        for l in range(NL):
            tau_eff = tau_line * profile[l]
            # outward
            ILambda = sc_2nd_order(tau_eff, S, mu[m], B_boundary) # Use B at boundary
            J += ILambda[0] * profile[l] * wx[l] * wmu[m] * 0.5
            L += ILambda[1] * profile[l] * wx[l] * wmu[m] * 0.5

            # inward
            ILambda = sc_2nd_order(tau_eff, S, -mu[m], 0.0) # Zero boundary at top
            J += ILambda[0] * profile[l] * wx[l] * wmu[m] * 0.5
            L += ILambda[1] * profile[l] * wx[l] * wmu[m] * 0.5
    return J, L # returns J and L with shape (ND,)